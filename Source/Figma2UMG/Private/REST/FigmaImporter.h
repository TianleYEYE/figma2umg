...
protected:
	...
	bool IsRateLimitResponse(FHttpResponsePtr HttpResponse) const;
	bool RetryRateLimitedRequest(const FString& MessagePrefix, int& RetryCount, TFunction<void()> RetryCallback);
	...
	FString PendingImageURLRequestFileKey;
	TArray<FString> PendingImageURLRequestIds;
	int ImageDownloadCount = 0;
	int ImageDownloadCountTotal = 0;
	int ImageURLRequestedCount = 0;
	int TotalImageURLRequestedCount = 0;
	int FileRequestRetryCount = 0;
	int LibraryFileRequestRetryCount = 0;
